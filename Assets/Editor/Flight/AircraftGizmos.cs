...
public static class AircraftGizmos
{
    ...
    public static void AircraftPhysicsGizmos(FlightModel phys, GizmoType gizmoType)
    {
    ...
    public static void SurfaceGizmos(AeroSurface surf, GizmoType gizmoType)
    {
        ...
        if (settings.showSurfaces)
        {
            ...
            DrawSurface(surf.transform, surf.Config, surf.FlapAngle, surf.IsStalling);
        }
        ...
        if (settings.showForces)
        {
            ...
            DrawForces(surf.transform, surf.Forces.Lift * scale, surf.Forces.Drag * scale, surf.Forces.Torque * scale);
        }
    }
    ...
    private static void DrawSurface(Transform transform, AeroSurfaceConfig config, float flapAngle, bool isAtStall = false)
    {
        ...
        DrawRectangle(transform.position + transform.forward * (0.25f * config.Chord - 0.5f * mainChord),
                transform.rotation,
                config.Span,
                mainChord,
        ...
        if (config.FlapFraction > 0)
        {
            DrawRectangle(transform.position
                + transform.forward * (0.25f * config.Chord - mainChord - 0.02f - 0.5f * flapChord * Mathf.Cos(flapAngle))
                - transform.up * 0.5f * Mathf.Sin(flapAngle) * flapChord,
                    transform.rotation * Quaternion.AngleAxis(flapAngle * Mathf.Rad2Deg, -Vector3.right),
                    config.Span,
                    flapChord,
    ...
    {
        Vector3[] vertices = new Vector3[4];
        vertices[0] = new Vector3(width, 0, -height) * 0.5f;
        vertices[1] = new Vector3(width, 0, height) * 0.5f;
        vertices[2] = new Vector3(-width, 0, height) * 0.5f;
        vertices[3] = new Vector3(-width, 0, -height) * 0.5f;
        for (int i = 0; i < 4; i++)
        {
